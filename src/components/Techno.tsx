...
// Affiche le texte en colorant les termes techniques {{terme}} en yellow-500
function renderDescriptionWithHighlight(text: string) {
  const parts = text.split(/(\{\{[^}]+\}\})/g);
  return parts.map((part, i) => {
    if (part.startsWith('{{') && part.endsWith('}}')) {
      return <span key={i} className="text-yellow-500">{part.slice(2, -2)}</span>;
    }
...
export default function TechSkills() {
  ...
  const frontendSkills: Skill[] = [
    ...
    { name: 'Angular', level: 92, icon: getTechIcon('Angular').icon, color: 'from-red-600 to-red-700', iconType: getTechIcon('Angular').type },
    { name: 'Zustand', level: 90, icon: getTechIcon('Zustand').icon, color: 'from-purple-500 to-purple-600', iconType: getTechIcon('Zustand').type },
    { name: 'NgRx', level: 88, icon: getTechIcon('NgRx').icon, color: 'from-purple-600 to-purple-700', iconType: getTechIcon('NgRx').type },
    { name: 'Styled component', level: 88, icon: getTechIcon('Styled component').icon, color: 'from-pink-500 to-pink-600', iconType: getTechIcon('Styled component').type },
    { name: 'Optimisation SEO', level: 90, icon: getTechIcon('Optimisation SEO').icon, color: 'from-green-500 to-green-600', iconType: getTechIcon('Optimisation SEO').type }
  ];

  const backendSkills: Skill[] = [
    { name: 'ExpressJs', level: 92, icon: getTechIcon('ExpressJs').icon, color: 'from-gray-700 to-gray-800', iconType: getTechIcon('ExpressJs').type },
    { name: 'Laravel', level: 90, icon: getTechIcon('Laravel').icon, color: 'from-red-500 to-red-600', iconType: getTechIcon('Laravel').type },
    { name: 'NestJs', level: 90, icon: getTechIcon('NestJs').icon, color: 'from-red-600 to-pink-600', iconType: getTechIcon('NestJs').type },
    { name: 'Dotnet', level: 88, icon: getTechIcon('Dotnet').icon, color: 'from-purple-600 to-purple-700', iconType: getTechIcon('Dotnet').type },
    { name: 'FastApi', level: 88, icon: getTechIcon('FastApi').icon, color: 'from-teal-500 to-teal-600', iconType: getTechIcon('FastApi').type },
    { name: 'Sql Server', level: 88, icon: getTechIcon('Sql Server').icon, color: 'from-gray-600 to-gray-700', iconType: getTechIcon('Sql Server').type },
    { name: 'Postgres', level: 90, icon: getTechIcon('Postgres').icon, color: 'from-blue-600 to-blue-700', iconType: getTechIcon('Postgres').type },
    { name: 'Qdrant', level: 85, icon: getTechIcon('Qdrant').icon, color: 'from-pink-600 to-red-600', iconType: getTechIcon('Qdrant').type },
    { name: 'Redis', level: 88, icon: getTechIcon('Redis').icon, color: 'from-red-700 to-red-800', iconType: getTechIcon('Redis').type }
  ];
  ...
  const designTools: Skill[] = [
    ...
    { name: 'Figma', level: 92, icon: getTechIcon('Figma').icon, color: 'from-purple-500 to-pink-500', iconType: getTechIcon('Figma').type },
    { name: 'Adobe Photoshop', level: 88, icon: getTechIcon('Adobe Photoshop').icon, color: 'from-blue-600 to-blue-700', iconType: getTechIcon('Adobe Photoshop').type },
    { name: 'Adobe Illustrator', level: 82, icon: getTechIcon('Adobe Illustrator').icon, color: 'from-orange-600 to-orange-700', iconType: getTechIcon('Adobe Illustrator').type }
  ];
  ...
  const devTools: Skill[] = [
    ...
    { name: 'Jira', level: 92, icon: getTechIcon('Jira').icon, color: 'from-blue-600 to-blue-700', iconType: getTechIcon('Jira').type },
    { name: 'Jest', level: 90, icon: getTechIcon('Jest').icon, color: 'from-red-700 to-red-800', iconType: getTechIcon('Jest').type },
    { name: 'LangChain', level: 85, icon: getTechIcon('LangChain').icon, color: 'from-green-600 to-green-700', iconType: getTechIcon('LangChain').type },
    { name: 'LLM', level: 85, icon: getTechIcon('LLM').icon, color: 'from-blue-500 to-blue-600', iconType: getTechIcon('LLM').type },
    { name: 'Docker', level: 88, icon: getTechIcon('Docker').icon, color: 'from-blue-600 to-blue-700', iconType: getTechIcon('Docker').type }
  ];
  ...
  const SkillCard = ({ skill, index }: { skill: Skill; index: number }) => (
    ...
    <div
      ...
      className="relative ilo bg-gray-800/20 backdrop-blur-[3px] rounded-lg p-2 md:p-5 border border-gray-700 hover:border-yellow-500 transition-all duration-300 transform hover:scale-105 hover:shadow-xl hover:shadow-yellow-500/20"
    >
      ...
      <div className="w-full ilo bg-gray-800/90 rounded-full h-0.5 overflow-hidden">
        <div
  ...
  const InfoBox = ({  description }: { description: string }) => (
    <div className="ilo bg-gray-800/20 backdrop-blur-[3px] rounded-lg p-6 border border-gray-700">
     <p className="text-gray-300 leading-relaxed text-sm">{renderDescriptionWithHighlight(description)}</p>
    </div>
  );

  const InfoBox1 = ({  description }: { description: string }) => (
    <div className="ilo bg-gray-800/20 backdrop-blur-[3px] rounded-lg p-6 border border-gray-700">
     <p className="text-gray-300 leading-relaxed text-sm">{renderDescriptionWithHighlight(description)}</p>
    </div>
  );
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <ScrollReveal direction="up" delay={0.4} duration={0.8}>
          ...
          <div className='block md:hidden'>
            ...
            <div className='md:mt-40 block md:hidden'>
              <ScrollReveal direction="left" delay={0.6} duration={0.8}>
                <InfoBox
                  description={t('techno.frontendDesc1')}
                />
              </ScrollReveal>
            </div>
            ...
            <div className='md:mt-40 md:block hidden'>
              <ScrollReveal direction="left" delay={0.6} duration={0.8}>
                <InfoBox
                  description={t('techno.frontendDesc1')}
                />
        ...
        <ScrollReveal direction="up" delay={0.5} duration={0.8}>
          ...
          <div className='md:mt-40'>
              <ScrollReveal direction="right" delay={0.7} duration={0.8}>
                <InfoBox
                   description={t('techno.backendDesc1')}
                />
              </ScrollReveal>
            </div>
            <div>
          ...
          <div className="grid lg:grid-cols-4 gap-8 mb-16">
            ...
            <div>
              <ScrollReveal direction="left" delay={0.8} duration={0.8}>
                <InfoBox
                  description={t('techno.designDesc1')}
                />
              </ScrollReveal>
            </div>
            <div className='md:mt-40'>
        ...
        <ScrollReveal direction="up" delay={0.7} duration={0.8}>
          <div className="grid lg:grid-cols-4 gap-8">
            ...
            <div>
              <ScrollReveal direction="right" delay={0.9} duration={0.8}>
                <InfoBox
                 description={t('techno.outilsDesc2')}
                />
              </ScrollReveal>
            </div>
          </div>
